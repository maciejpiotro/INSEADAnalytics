...
<br>
(1)
6 components are required to explain at least 50% of variance (they will explain 52.29% of variance, 5 components would explain only 48.57%)
<br>
(2)
In Component 1, the following factors have the highest weights: 5 yr T-Notes US	and 10yr T-Notes. Both have the weight of 0.93
<br>
(3)
...
2 Currencies
3 European Indices
4 North American Indices (US and Canada)
5 Futures based on fixed income instruments
6 Oil
7 Metals
8 Agriculture products (seeds and grains)
9 Precious metals
10 Asian (Japan and HK) indices
...
<br>
(5) 
During pre-crisis years 6 Components are required to explain 50% of variance. During crisis years only 4 Components. It shows that during bear years markets are more correlated because same number of components explain higher proportion of variance. 
<br>
...
<br>
1. The code above takes moving averages of futures for windows with different length (10 days, 30 days, etc). Subsequently it creates matrix that contains signals - i.e. sum of moving averages within a given window. The weights are scaled down to ensure that $1 is invested each day. At the end the trade decision is based on the signal that is shifted by two days.
<br>
<br> 
2. The MAfreq<-seq(10,250,by=20) determines the sequence of windows that are taken into consideration. The formula determines starting window, frequency of windows,and largest window.
